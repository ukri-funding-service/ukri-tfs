...
describe('packages/message-handling - awsChannel/inbound/sqsQueue', () => {
    let sqs: SQSClient;
    ...
    describe('receive', () => {
        it('deletes messages from queue when they have a receipt handle', async () => {
            const receivedMessages: ReceiveMessageResult = {
                Messages: [
                    { MessageId: '1', Body: '{"type": "my-id", "data":{}}', ReceiptHandle: 'some-handle' },
                    { MessageId: '2', Body: '{"type": "my-id2", "data":{}}', ReceiptHandle: '' },
                ],
            };
        ...
        it('rejects on queue read failures with Error', async () => {
            ...
            const queue = new SqsQueue(sqs, queueConfig, logger);
            await expect(queue.receive()).to.be.rejectedWith(Error);
        });
