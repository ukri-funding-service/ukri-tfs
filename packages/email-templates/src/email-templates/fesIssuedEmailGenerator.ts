...
export class FesIssuedEmailGenerator extends EmailGenerator<FesIssuedEmailData> {
    ...
    private emailText = {
        ...baseEmailText,
        getDeadlineText: (deadlineDays?: number, deadlineDate?: Date) =>
            `You have ${deadlineDays ?? '90'} days to complete the fES and return it to us.${
                deadlineDate ? ` This means you must submit your FES by ${this.formatDate(deadlineDate)}` : ''
            }`,
    ...
    generateHtml(emailData: FesIssuedEmailData): string {
        return generateTemplatedEmail(
            ...
            ${generateEmailParagraph(this.emailText.getDeadlineText(emailData.deadlineDays, emailData.deadlineDate))}
    ...
    generateText(emailData: FesIssuedEmailData): string {
        ...
        ${this.emailText.getDeadlineText(emailData.deadlineDays, emailData.deadlineDate)}
